...
public class WordCount {
    ...
    private static void runner(PigServer server, String inputFile, String outputFile) throws IOException, URISyntaxException {
        server.registerJar(new File(WordCount.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getPath());
        InputStream stream = WordCount.class.getResourceAsStream("WordCount.pig");
        if (stream == null) {
            throw new IllegalStateException("Unable to load resource 'WordCount.pig'");
        }
